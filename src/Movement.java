...
public class Movement extends Thread {
	 ...
	 * straight.
	 */
	public void changeBaseSpeedLeft(int change) {
		this.baseSpeedLeft+=change;
	}
	 ...
	 * straight.
	 */
	public void changeBaseSpeedRight(int change) {
		this.baseSpeedRight+=change;
	}
	...
	/**
	 * Method used for driving straight and sticking to the wall on the side.
	 * TODO ITS FUCKED UP
	 * @param distance
	 *            Distance from the wall to stick to.
	 */
	public void holdTheWall(int distance) {
		SoundSensor soundSensor = new SoundSensor(SensorPort.S3);
		UltrasonicSensor distanceSensor = new UltrasonicSensor(SensorPort.S4);

		int startingDistance = distanceSensor.getDistance();
		MotorPort.C.controlMotor(70, 1);
		MotorPort.B.controlMotor(70, 1);
		
		
		while (Button.ENTER.isUp()) {
			if (distanceSensor.getDistance() > startingDistance + 6)
			{
				MotorPort.B.controlMotor(69, 1);
				System.out.println("ZBLIZAM SIE");
			}
			else if (distanceSensor.getDistance() < startingDistance - 6)
			{
				stopReset();				
				MotorPort.B.controlMotor(71, 1);
				System.out.println("ODDALAM SIE");
			}
