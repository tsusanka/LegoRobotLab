...
public class Movement extends Thread
{
    ...
    * 
    * @param wheelsRadius
    *           Radius of installed wheels in centimeters. Possible choices are: 4. TODO Wheel sizes. Add another
    *           possible sizes of the wheels.
    */
   public Movement(double wheelsRadius)
   {
   ...
   /**
    * Use this when robot is turning slightly right while he was supposed to go straight.
    */
   ...
   /**
    * Use this when robot is turning slightly left while he was supposed to go straight.
    */
   ...
   {
      centimetersDistance = (360*centimetersDistance)/(wheelsRadius*Math.PI*2);
      MotorPort.B.controlMotor(baseSpeedRight, centimetersDistance > 0 ? 1 : 2);
      MotorPort.C.controlMotor(baseSpeedLeft, centimetersDistance > 0 ? 1 : 2);

      while (true)
      {
         if (Math.abs(MotorPort.B.getTachoCount()) > Math.abs(centimetersDistance))
            break;
      }
   ...
   /**
    * Starts both motors for an unspecified time. They need to be then stopped by stopMotors or stopReset methods.
    * 
    * @param powerModificator
    *           Modifies the standard power of motors. Value under -30 wont start the motors at all, while value over 30
    *           gives them maximum power.
    * @param backwards
    *           Value 1 if you want to run motors backwards. Otherwise it has to be 0.
    */
   public void startMotors(int powerModificator, int backwards)
   {
      MotorPort.B.controlMotor(powerModificator + baseSpeedRight, backwards + 1);
      MotorPort.C.controlMotor(powerModificator + baseSpeedLeft, backwards + 1);
   }
    ...
    * 
    * @param angle
    *           Angle to turn the robot. Negative value turns left and positive turns right.
    */
   ...
   /**
    * Moves the robot in a spiral path. TODO Spiral size. Add arguments to this method so you can pick the length/end
    * radius of the spiral and if it should move from the inside to the outside or vice versa
    */
   private void spiral()
   {
      int degree = 0;
   ...
   /**
    * Moves the robot in a circle path. TODO Circle radius. Add argument to this method so you can pick a radius of the
    * circle.
    */
   private void circle()
   {
   ...
   /**
    * Moves the robot in a custom, given path. The pattern is going straight for a given distance, then turning for a
    * given angle and then again from the beginning. This path can be endless. Argument vectors should have the same
    * size.
    * 
    * @param moveVector
    ...
    */
   private void customMove(Vector<Integer> moveVector, Vector<Integer> turnVector)
   {
      ...
      for (int i = 0; i < moveVector.size(); i++)
      {
         ...
         // run straight
         while (Math.abs(MotorPort.B.getTachoCount()) < Math.abs(moveVector.elementAt(i)) * 300)
         {
         ...
         {
            while (Math.abs(MotorPort.B.getTachoCount()) < Math.abs(turnVector.elementAt(i)) * 55)
               MotorPort.B.controlMotor(70, 1);
         }
         else
         {
            while (Math.abs(MotorPort.C.getTachoCount()) < Math.abs(turnVector.elementAt(i)) * 55)
   ...
   /**
    * Moves the robot in a random path. It uses the distance sensor not to crash into a wall. The pattern is moving
    * straight until almost hitting a wall, then turning random angle and then going straight again. Just like
   ...
   /**
    * If DEBUG_MODE == true
    * Changeable contents. Test whatever you want inside
    */
   ...
   /**
    * If DEBUG_MODE == true
    * Print the battery voltage in volts on the screen.
    */
   ...
   /**
    * If DEBUG_MODE == true
    * Test if TachoCounter is set up correctly. You should manually check if wheel made a full rotation
    */
